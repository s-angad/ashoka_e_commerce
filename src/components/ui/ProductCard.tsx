...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-2xl border border-stone-200/80 shadow-xs hover:shadow-xl hover:border-stone-300 transition-all duration-300 overflow-hidden flex flex-col justify-between relative"
    >
      {/* Top Image Container */}
      <div className="relative aspect-square w-full bg-stone-100/70 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10 pointer-events-none">
          {product.discountBadge && (
            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-[#C59B27] text-stone-950 shadow-xs">
              {product.discountBadge}
            </span>
          )}
          {product.isBestSeller && (
            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold bg-[#1C3A27] text-amber-200 shadow-xs">
              ★ Best Seller
            </span>
          )}
        </div>
        ...
        {product.organicCertified && (
          <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-xs px-2 py-0.5 rounded-md text-[10px] font-medium text-emerald-800 border border-emerald-200/60">
            100% Organic
          </div>
        )}
      </div>
      ...
      <div className="p-4 flex flex-col flex-1 justify-between gap-3">
        <div>
          ...
          <div className="flex items-center justify-between text-xs text-stone-500 mb-1">
            <span className="font-medium tracking-wide text-ashoka-sage uppercase text-[10px]">
              {product.category}
            </span>
            {product.hindiName && (
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow-xs ${
              isAdded
                ...
                : 'bg-[#1C3A27] hover:bg-[#244833] text-amber-100 hover:text-white'
            }`}
          >
            {isAdded ? (
              <>
